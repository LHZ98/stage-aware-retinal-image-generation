...
# 输入和输出路径（可以从命令行参数获取）
INPUT_DIR="/home/yuganlab/Desktop/Haozhe/MICCAI2026/datasets/datasets/the-sustechsysu-dataset/SYSU"  # 从命令行参数获取，或使用默认路径
OUTPUT_DIR="/home/yuganlab/Desktop/Haozhe/MICCAI2026/nnUNet/predictions/SYSU"    # 预测结果输出路径

# 指定要使用的fold（可选，默认使用所有已训练的folds）
# 如果只想用单个fold，设置为 "0", "1", "2" 等
# 如果设置为 "all"，会使用所有已训练的folds进行集成
# 如果设置为空字符串 ""，会自动检测所有可用的folds
SPECIFIC_FOLD="0"  # 第三个参数：指定fold，如 "0" 表示只用fold_0
...
if [ ! -d "${INPUT_DIR}" ]; then
    ...
    echo ""
    echo "使用方法:"
    echo "  bash test_HRF.sh <输入图像文件夹> [输出文件夹] [fold编号]"
    echo ""
    echo "参数说明:"
    echo "  输入图像文件夹: 包含测试图像的文件夹（必需）"
    echo "  输出文件夹: 预测结果保存路径（可选，默认: ./predictions/HRF）"
    ...
    echo ""
    echo "示例:"
    echo "  bash test_HRF.sh /path/to/test/images ./predictions"
    echo "  bash test_HRF.sh /path/to/test/images ./predictions 0    # 只用fold_0"
    echo "  bash test_HRF.sh /path/to/test/images ./predictions all  # 使用所有folds"
    echo ""
    exit 1
fi
