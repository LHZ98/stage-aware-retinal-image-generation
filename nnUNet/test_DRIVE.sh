...
# 输入和输出路径（可以从命令行参数获取）
INPUT_DIR="${1:-/home/yuganlab/Desktop/Haozhe/MICCAI2026/datasets/masks/DRIVE/test/images}"  # 从命令行参数获取，或使用默认路径
OUTPUT_DIR="${2:-./predictions/DRIVE}"    # 预测结果输出路径
...
if [ ! -d "${INPUT_DIR}" ]; then
    ...
    echo ""
    echo "使用方法:"
    echo "  bash test_DRIVE.sh <输入图像文件夹> [输出文件夹] [fold编号]"
    echo ""
    echo "参数说明:"
    echo "  输入图像文件夹: 包含测试图像的文件夹（必需）"
    echo "  输出文件夹: 预测结果保存路径（可选，默认: ./predictions/DRIVE）"
    ...
    echo ""
    echo "示例:"
    echo "  bash test_DRIVE.sh /path/to/test/images ./predictions"
    echo "  bash test_DRIVE.sh /path/to/test/images ./predictions 0    # 只用fold_0"
    echo "  bash test_DRIVE.sh /path/to/test/images ./predictions all  # 使用所有folds"
    echo ""
    exit 1
fi
