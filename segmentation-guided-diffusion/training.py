...
def _get_ref_same_label(images, labels, device):
    """For each sample i, pick another j with same label (j != i); ref_images[i] = images[j]. Same device/dtype as images."""
    B = images.shape[0]
    labels_np = labels.cpu().numpy() if labels.dim() > 0 else np.array([labels.item()])
    if labels_np.size != B:
        labels_np = np.broadcast_to(labels_np, B)
    indices = np.arange(B)
    ref_indices = np.zeros(B, dtype=np.int64)
    for i in range(B):
        same = np.where((labels_np == labels_np[i]) & (indices != i))[0]
        if len(same) > 0:
            ref_indices[i] = np.random.choice(same)
        else:
            other = np.where(indices != i)[0]
            ref_indices[i] = np.random.choice(other) if len(other) > 0 else i
    ref_images = images[torch.from_numpy(ref_indices).to(device)]
    return ref_images
...
class TrainingConfig:
    ...
    # ref conditioning: 3-channel ref (same label), downscale then upsample before concat
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if getattr(config, "use_ref_conditioning", False):
                # ref: within-batch same label; downscale then upsample
                labels_b = batch["labels"]
                if not isinstance(labels_b, torch.Tensor):
                    labels_b = torch.tensor(labels_b, dtype=torch.long, device=clean_images.device)
                else:
                    labels_b = labels_b.to(clean_images.device)
                ref_images = _get_ref_same_label(clean_images, labels_b, device)
                ref_images = F.interpolate(
                    ref_images,
                    size=(config.ref_downsize, config.ref_downsize),
                    mode="bilinear",
                    align_corners=False,
                )
                ref_images = F.interpolate(
                ...
                noisy_images = torch.cat((noisy_images, ref_images), dim=1)
        ...
        if (epoch + 1) % config.save_image_epochs == 0 or epoch == config.num_epochs - 1:
            if config.segmentation_guided:
                ...
                if getattr(config, "use_ref_conditioning", False) and "labels" in seg_batch:
                    imgs = seg_batch["images"].to(device)
                    lbls = seg_batch["labels"] if isinstance(seg_batch["labels"], torch.Tensor) else torch.tensor(seg_batch["labels"], dtype=torch.long, device=device)
                    seg_batch["ref_images"] = _get_ref_same_label(imgs, lbls, device)
        ...
        if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
            try:
                # Use safe_serialization=False to avoid safetensors.torch error
                # This uses PyTorch .bin format instead of safetensors format
                pipeline.save_pretrained(config.output_dir, safe_serialization=False)
            except Exception as e:
                # If save fails for any reason, log warning but continue training
                print(f"Warning: Failed to save model checkpoint: {e}")
                print("Training will continue, but model may not be saved.")
                import traceback
                traceback.print_exc()
