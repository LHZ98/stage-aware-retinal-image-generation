...
def add_segmentations_to_noise(noisy_images, segmentations_batch, config, device):
    """
    concat segmentations to noisy image; optionally concat ref (downscale then upsample) when use_ref_conditioning.
    """
    ...
    if getattr(config, "use_ref_conditioning", False) and "ref_images" in segmentations_batch:
        ref = segmentations_batch["ref_images"].to(device=device, dtype=noisy_images.dtype)
        ref_downsize = getattr(config, "ref_downsize", 64)
        ref = F.interpolate(ref, size=(ref_downsize, ref_downsize), mode="bilinear", align_corners=False)
        ref = F.interpolate(ref, size=(noisy_images.shape[2], noisy_images.shape[3]), mode="bilinear", align_corners=False)
        noisy_images = torch.cat((noisy_images, ref), dim=1)
...
class SegGuidedDDPMPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # image channels to denoise (first out_channels); rest are cond (mask, optional ref)
        if getattr(self.external_config, "use_ref_conditioning", False):
            img_channel_ct = self.unet.config.out_channels
        elif self.external_config.segmentation_channel_mode == "single":
...
class SegGuidedDDIMPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if isinstance(self.unet.config.sample_size, int):
            ...
            image_shape = (batch_size, img_channel_ct, *self.unet.config.sample_size)
